...
echo Building space-map plugin ...
...
cl /nologo /EHsc /std:c++20 /O2 /Zi /Fd:space-map.pdb /utf-8 /MT /DUNICODE /D_UNICODE /LD space_map.cpp ^
   /link /OUT:space-map.dll gdiplus.lib shlwapi.lib user32.lib gdi32.lib shell32.lib ole32.lib uuid.lib "..\..\xunjieso.lib"
if errorlevel 1 (
    echo Build failed!
    pause
    exit /b 1
...
:: instance; force-kill skips the DB flush (accepted trade-off). System32
:: absolute paths: a bare `find` would hit GNU find from Git Bash's PATH.
...
:: Deploy to the runtime plugins dir (exe dir \ plugins \ space-map)
if not exist "..\..\plugins\space-map" mkdir "..\..\plugins\space-map"
copy /y space-map.dll "..\..\plugins\space-map\space-map.dll" >nul
if errorlevel 1 (
    echo ERROR: deploy failed - plugins\space-map\space-map.dll is locked.
    pause
    exit /b 1
)
